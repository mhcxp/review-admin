...
public class TaskTypeResource extends BaseResource<TaskType>{
	
	private static final Logger logger = LoggerFactory.getLogger(TaskTypeResource.class);
	
	private final TaskTypeDAO dao;
	
	public TaskTypeResource(TaskTypeDAO dao){
		this.dao = dao;
	}

	@Override
	public BaseDAO<TaskType> getBaseDAO() {
		return this.dao;
	}
	...
	public Message get(@PathParam("pid") long pid){
		try {
			TaskType r = this.dao.getById(pid);
			return this.ok(r);
		} catch (Exception e) {
	...
	public Message all(){
		try {
			List<TaskType> result = this.dao.findAll();
	...
	public Message add(TaskType type){
		try {
			TaskType r = this.dao.saveOrUpdate(type);
			return this.ok(r);
		} catch (BusinessException e) {
			logger.error(e.getMessage());
	...
	@Consumes(MediaTypeUTF8.APPLICATION_JSON_UTF8)
	@UnitOfWork(transactional=true)
	public Message edit(TaskType type){
		try {
			TaskType source = this.dao.getById(type.getPid());
			source.setDescription(type.getDescription()).setName(type.getName());
			TaskType r = this.dao.saveOrUpdate(source);
			return this.ok(r);
		} catch (BusinessException e) {
			logger.error(e.getMessage());
			return this.fail(e.getMessage());
