...
function ngGridCsvExportPlugin (opts) {
    ...
    opts.linkLabel = opts.linkLabel || '导出CSV';
...
//            var fp = grid.$root.find(opts.containerPanel);
           ...
           /*
            var csvDataLinkHtml = '<span class="' + opts.linkClass + '"><br>';
...
//            fp.append(csvDataLinkHtml);
            var container = $(opts.containerPanel);
        	var html  = '<a href="data:text/csv;charset=UTF-8,\ufeff';//加入了\ufeff从而达到中文不乱码
        	html += encodeURIComponent(csvData);
        	html += '" download="' + opts.fileName + '">' + opts.linkLabel + '</a>';
        	container.html(html);
        }
        setTimeout(showDs, 0);
